...
**BUILD 53 (queued, founder call 2026-09-10): large-screen / orientation support.** Remove the portrait-only lock (app.json `orientation: "portrait"` → allow rotation/resize) and make every screen work in landscape + on foldables/tablets. Driven by a Google Play advisory: Android 16 will IGNORE the PORTRAIT restriction on large screens (foldables/tablets), so unhandled layouts break for those users (founder is on a Z Fold 7). Real work — responsive layouts across all screens + landscape/large-screen testing. NOT urgent, NOT a blocker (advisory only). The two sibling Play advisories (deprecated edge-to-edge window-color APIs; manual image download/decode) are THIRD-PARTY LIBRARY internals (Fresco/Glide/canhub-cropper) — `edgeToEdgeEnabled: true` already set; they clear via future Expo SDK bumps, no app-level action.
