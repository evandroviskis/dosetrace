...
**1.2.2 CRITICAL AUTH BUG FOUND + FIXED, REBUILDING (2026-09-21).** Founder device-tested on sim: account delete "did not work." Diagnosed via sim (bundle-swap): `lock: processLock` (added to lib/supabase.js as belt-and-suspenders for the SecureStore adapter) DEADLOCKED `supabase.auth.signOut()` — the session was never cleared (persisted across relaunch), the app never routed to welcome, so BOTH Sign out AND Delete account silently did nothing. Removed processLock (commit 583339b); the two-slot atomic SecureStore adapter already made it unnecessary (reverts to 1.2.1 no-lock behavior). VERIFIED on sim via bundle-swap: sign-out now routes to the welcome screen. Builds 58/vc40 (on TestFlight/internal) have the bug — DO NOT promote them; superseded by the rebuild. Founder chose to FOLD the blood-test-report delete feature (19cd2b6) into this 1.2.2 rebuild. Rebuilding both platforms off main HEAD (→ iOS build 59 (VALID, TestFlight) / Android vc41 (internal, completed)) auto-submitted to internal/TestFlight. The delete feature still needs its own dt-council + ship-check before PRODUCTION promotion. LESSON: the deadlock was runtime-only, invisible to all 11 static reviewers (who even recommended processLock) — the on-device QA gate is what caught it. --- Prior (superseded) build note below:
**1.2.2 BUILT + ON INTERNAL/TESTFLIGHT — AWAITING FOUNDER DEVICE QA (2026-09-19).** eas build both platforms + auto-submit done: iOS build 58 (c4614dfe) proc=VALID → internal TestFlight "Team (Expo)" (installable now; no external beta review needed for owner); Android vc40 on INTERNAL track, status=completed (installable via internal opt-in link). Production untouched (Android 1.2.1 vc39 completed; iOS 1.2.1 READY_FOR_DISTRIBUTION) — "halt" was moot, both 1.2.1 already fully live/completed; 1.2.2 supersedes on promotion AFTER device QA. NEXT (founder): device-QA gate then promote. Do NOT promote to Android production / iOS App Store review until the QA passes (Android has no downgrade). To promote later: Android internal→production (staged), iOS = attach build 58 to a new 1.2.2 App Store version + What's New + Submit. DEVICE-QA (critical first): install 1.2.2 over live 1.2.1 on Android → LOCK phone → cold-launch → still signed in + data intact (keychainAccessible fix); then token refresh >1h, sign-in email/Google/Apple, delete ×3 (Apple revoke 200/manual, Google, email), external Apple revoke → same-account re-sign-in restores data, sign-out→different account no bleed, new alerts both themes. --- Earlier: dt-council GO-WITH-CHANGES + ship-check PASS; A=internal, B=delete-always-wins. Founder decisions: A=Android internal track, B=delete-always-wins revoke (both confirmed). Council fixes applied (commit f37a96a + eas.json internal): SecureStore keychainAccessible=AFTER_FIRST_UNLOCK (QA+journey blocker — WHEN_UNLOCKED default nulled locked-state reads → dropped session) + byte-based chunking (senior-eng — multibyte name > 2KB byte limit → silent plaintext fallback); app.json 1.2.1→1.2.2 (+ removed dead buildNumber:2); removed stale email-only settings_delete_msg ×6 (regulatory 5.1.1(v)); simplified Apple manual-revoke note ×6 (dropped version-fragile 5-level Settings path); privacy-policy Your-Rights clause now states full-data deletion; new test clearLocalDatabase ⊇ syncCore.TABLES (shared-device leak guard); eas.json android production/10%→internal. 178 tests (177 pass + known energy.test.ts), i18n parity 5/5, iOS bundle clean, expo config 1.2.2, secrets scan clean, Apple sign-in button present (4.8 parity). Ship-check: Gate A pass; Gate B auth invariant intact (onAuthStateChange synchronous) — the migration/refresh/delete checks are the owner device-QA post-build; Gate C internal/TestFlight only + version bumped; Gate D guardrails intact. PRE-SUBMIT (founder, before Android eas submit): halt or complete the 1.2.1 vc39 10% production rollout (Play allows one active production rollout). DEVICE-QA GATE (owner, post-build, before any promote): update over live 1.2.1 on Android → LOCK phone → cold-launch → still signed in + data intact (the WHEN_UNLOCKED fix); token refresh >1h; sign-in email/Google/Apple; delete ×3 (Apple revoke 200/manual, Google, email); external Apple revoke → same-account re-sign-in restores data; sign-out → different account = no bleed; new alerts both themes. Council: 1.2.3 returns to calculator/Journey moat, not more hardening.


**SECURITY HARDEN (Grok handoff 2026-09-19) — P0/P2 DONE+LIVE, P1 CODE-DONE (device-QA gated).** P0 RLS: audited all 17 public tables (RLS on everywhere; docs/security-rls-audit-2026-09-19.md); dropped referral_codes public UPDATE hole (USING(true) — redemption uses the SECURITY DEFINER redeem_referral_code RPC which exists live), added explicit WITH CHECK to vaccines UPDATE (migration rls_harden_referral_and_vaccines); versioned core RLS (supabase_core_tables_rls.sql); cross-user test PROVEN live (user A sees 0 of B's rows; apple_tokens = permission-denied to authenticated). The old world-readable referral SELECT vuln is NOT present live. P2: iOS UIFileSharingEnabled+LSSupportsOpeningDocumentsInPlace -> false (Documents held the SQLite DB + export JSON; export=share sheet, upload=picker, nothing depends on file sharing); new config plugin plugins/withAndroidBackupDisabled.js -> android:allowBackup=false (CNG, prebuild); web/adm one-time prompt fallback so admin token no longer needs a URL (URL already stripped via replaceState + sessionStorage + 401 fail-closed); npm audit triaged (docs/security-npm-audit-2026-09-19.md — all 2 critical/17 high are build-toolchain, none shipped; fix = next Expo SDK bump, NOT npm audit fix on the Expo-managed tree). P1 Secure Store (commit 3ed4278): auth.storage AsyncStorage -> chunked expo-secure-store adapter (lib/secureStore.js) — encrypted at rest, chunks the ~2KB limit, one-time AsyncStorage->SecureStore migration (delete only after write succeeds), AsyncStorage fallback on Keychain failure so the session is NEVER dropped; onAuthStateChange untouched. Bundle exports clean, 176/177 tests. Commits: 4c47197 (P0), 07d608b (P2), 3ed4278 (P1). Backend review of the SecureStore adapter running. GATED (founder): P1 Secure Store + the Apple client-revoke changes ride the NEXT EAS build — need dt-council + ship-check Gate B + on-device QA (cold-start still signed in after update; token refresh; sign-out clears both stores; delete flow) + founder go. Council verdict holds store submit until P0 (done) + P1 Secure Store have device evidence. DEFERRED: Expo SDK bump (npm audit remediation) — separate ticket; SQLCipher / cert pinning / repo-private stay out (founder call).


**APPLE 5.1.1(v) DELETE + TOKEN REVOKE — SERVER DEPLOYED, CLIENT PENDING BUILD (2026-09-19).** SERVER LIVE: founder set all 4 APPLE_* edge secrets (Team A4X4HHAKG7, Key L4Y6L3DYC7, client io.outcom.dosetrace, .p8 from new SIWA key "DoseTrace SIWA Revoke"); apple_tokens table applied via apply_migration (RLS on, 0 policies — verified); apple-link deployed (v1) + delete-user redeployed (v15, both verify_jwt=true, self-contained inlined bundles — dashboard/MCP-safe, no _shared import); both smoke-tested 401 without auth. delete-user upgrade required founder deploy approval (auto-mode "Production Deploy" block). CLIENT STILL OWED (rides next EAS build, gated by dt-council + ship-check Gate B + founder go + device QA): authorizationCode capture + apple-link call (lib/supabase.js), addRevokeListener (App.js), manual-revoke guidance (SettingsScreen). Until the app build ships, no tokens get stored so revoke has nothing to act on (expected). Original build notes: Grok handoff: in-app delete must also REVOKE Sign in with Apple tokens (TN3194). Root finding: the app historically discarded `credential.authorizationCode` (only used identityToken via signInWithIdToken) → NO Apple refresh_token was ever stored → revoke impossible until backfilled. Built the full chain (commit pending): (1) NEW `supabase/functions/_shared/apple.ts` — ES256 client_secret JWT via Web Crypto (IEEE-P1363 sig, NOT DER — verified), exchangeAuthCode + revokeRefreshToken (best-effort states revoked/already_gone/transient_error/config_missing); getAppleConfig()=null when any APPLE_* secret unset. (2) NEW `apple-link` edge fn — exchanges code, upserts refresh_token into NEW service-role-only `apple_tokens` (RLS on, ZERO policies, revoke all from anon/authenticated); binds exchanged Apple sub to caller (anti-griefing); non-blocking, never fails sign-in. (3) `delete-user` — revokes before wipe, BEST-EFFORT/DELETE-ALWAYS-WINS (retry once on transient then delete anyway; 4xx=already-gone=done; missing secrets=skip) — deliberate deviation from Grok's literal "fail the delete" because fail-closed traps already-revoked users (GDPR Art.17; journey-review F1/F3). Also CLOSED a pre-existing leak: added reality_checks/calc_snapshots/calc_targets/ai_food_usage/ai_scan_usage/reminders/apple_tokens to the delete list (F4) + drift-guard test (syncCore.TABLES ⊆ delete list). (4) `lib/supabase.js` signInWithApple captures authorizationCode → non-blocking apple-link call. (5) `App.js` expo addRevokeListener → deferred local signOut (no wipe; not intentional). (6) SettingsScreen shows manual-revoke guidance (×6 langs) when appleManualRevokeNeeded. GATES DONE: journey-review (6 findings, all addressed/noted), backend review (APPROVE, no blockers), 177 tests (only known energy.test.ts fails), i18n parity green, iOS bundle exports clean. Deno NOT installed → edge TS relied on review. **BLOCKER (founder): set 4 Supabase Edge secrets — APPLE_CLIENT_ID=io.outcom.dosetrace, APPLE_TEAM_ID, APPLE_SIA_KEY_ID, APPLE_SIA_PRIVATE_KEY (.p8 from a Sign in with Apple key).** THEN (on go): apply supabase_apple_tokens_table.sql (via MCP apply_migration for version tracking), deploy apple-link + delete-user via `supabase functions deploy` (CLI bundles _shared/ — dashboard paste-box breaks the import), run dt-council + ship-check Gate B + both-theme check on the new alert, device QA (Apple sign-in→delete→confirm appleRevoked:true; reinstall→fresh account). NOTE: revoke only works for Apple users who sign in AGAIN post-ship (auto-backfill); legacy Apple users get the manual-guidance path (unavoidable — no token was ever stored). Deferred/noted: F5 degraded reinstall for legacy no-token cohort; F6 second-device residual local data (product decision). Follow-ups still open: blend per-vial ratio editor; GDPR Art.9 consent wording (counsel); Supabase PITR.


**iOS 1.2.1 SUBMITTED FOR REVIEW + EN ASO SUBTITLE LIVE-PENDING (2026-09-19).** Founder logged into ASC; via Chrome console: created iOS App Version 1.2.1, attached build 57 (VALID), pasted What's New in ALL 6 localizations (en-US, es-ES, pt-BR, fr-FR, de-DE, it), and clicked Add for Review → Submit for Review ("1 Item Submitted"). VERIFIED via ASC API: appStoreVersion 1.2.1 = WAITING_FOR_REVIEW, whatsNew present in all 6 locales. ASO (Grok Bot, EN only, metadata-only): EN subtitle changed "Peptides, labs, body & more" → **"Peptides, labs. No advice"** (25/30) in App Information → saved; VERIFIED via ASC API (appInfo WAITING_FOR_REVIEW carries the new subtitle; live 1.2.0 appInfo still shows the old one — the new subtitle goes live WITH 1.2.1's approval). Play EN short description already = "Private journal: peptides, supplements, labs, vaccines & body. Never interprets." (80/80), committed via Android Publisher API (edit 01511201142240164964, title/description/video preserved). NO other store fields changed (no screenshots/preview/icon/name/long-desc). Android 1.2.1 = vc39, submitted to production 10% staged (verified earlier). POST-ASO (Grok directive): hold creatives 2-4 weeks / ~1k EN impressions; track impression→product-page-view→install (read from ASC/Play analytics). WATCH: Apple review email (~24-48h); if rejected, fix+resubmit. Still owed (1.2.1 follow-ups): blend per-vial ratio EDITOR (founder wants next); Apple token revoke on delete (5.1.1(v)); GDPR Art.9 consent wording (counsel); Supabase PITR.


**ONBOARDING UNIFIED + APPLE/GOOGLE SIGN-IN VERIFIED (2026-09-14).** Live customer hit broken "Sign up with Apple" on the PUBLIC App Store app — which is still 1.0.1 (old browser-redirect OAuth). ROOT CAUSE = 1.0.1 only; the CURRENT build's social sign-in is native signInWithIdToken (Apple + Google) and works (auth.identities: 62 google + 8 apple real sign-ups; no lingering browser-OAuth for social). So the fix is shipping 1.2. Pulled the 1.2.0 ASC submission back to DEVELOPER_REJECTED at founder's request (via Claude-in-Chrome console; corrected the mistyped version 1.2.1→1.2.0, build 55 auto-linked). THEN founder reported two onboarding issues on TestFlight build 55: (1) two different "About you" layouts (nice OnboardingFlowScreen for email-first vs old CompleteProfileScreen gate for every social sign-in), (2) social sign-in / post-deletion users skip the nice onboarding and hit the old gate. FIX (commit this session, journey-reviewed pre-impl, F1–F6 addressed): OnboardingFlowScreen gained a SIGNED-IN MODE (App.js routes session-but-incomplete users to it) — prefills from the account, shows only missing steps, WRITES via updateUser on finish (stash path never fires once SIGNED_IN), sign-out escape, offline-error handling; DELETED CompleteProfileScreen.js (rebuild=replace). Goals unified: new lib/profileGoals.js (goalOptions, 18 goals alphabetized) shared by onboarding + Settings editor (onboarding had only 5). 174/175 tests, i18n parity green (+ob_finish_setup/ob_finish_sub x6), bundle clean. NOT BUILT YET — needs dt-council + ship-check + a NEW EAS build (still 1.2.0, higher build number) → TestFlight → founder device-tests Apple+Google sign-in on iOS/Android + the unified onboarding (new=full flow, returning=only gaps) + delete→resignup fresh → attach the new build to the 1.2.0 ASC version → Submit. OPEN (founder to decide): whether to also restyle the Settings "Edit profile" section to the newer look (goals there already unified; layout not).

**ABOUT-YOU DESIGN MATCH (2026-09-15, founder-approved "Now its good").** Founder wanted the paced onboarding "About you" step to look EXACTLY like the deleted CompleteProfile screen he still had on his (stale) TestFlight build — not just text. Ported the exact CompleteProfile design onto the paced step (commits 77080ce, 69ed253, 2880f74): intro paragraph (profile_step_sub_required) + "* All fields are required" legend + per-field red asterisks; divider + "About you" section header (s.section w/ top hairline); 4-across uniform month grid (s.mGrid/s.mChip rounded-rects, replacing the 6-wide pills); quiet uppercase micro-labels (11/700 textFaint); split BIRTH MONTH / BIRTH YEAR labels; field wrappers + left-aligned option rows; routine step wrapped to match. Reuses existing keys (profile_step_title/profile_sec_about/profile_birth_month|year, all 6 langs) — no new strings. Verified via bundle-swap on the driven sim; founder approved from screenshots. 174/175 tests, bundle exports clean. STILL the same NOT-BUILT gate as above: dt-council + ship-check (incl. DARK-theme/contrast pass — his reference was dark) + NEW 1.2.0 build (higher build number) → TestFlight → device test → attach to 1.2.0 ASC version → Submit (fixes the live 1.0.1 Apple-sign-in bug for public users). Redundant EAS simulator build 1e2f187b was cancelled after founder approved from screenshots.

**1.2.0 FINAL PRODUCTION RELEASE — IN FLIGHT (2026-09-15).** Founder chose to submit STRAIGHT to store review on BOTH stores, replacing live 1.0.1 (iOS)/current (Android), overriding the TestFlight-only rule (his explicit call). Also refreshed the ENTIRE Settings + Edit Profile to the onboarding layout language (commit 2eaf489: section headers w/ hairline dividers, uppercase micro-labels, 4-col month grid, soft-outline pills, card2 inputs; style-only). Ran dt-council (9 reviewers) + folded ship-check: CODE verdict = ship (senior-eng ship, backend approve, journey PASS — the prior "deleted account recognized as returning" bug is NOT reproducible; qa "structurally sound, tests green"). PLAN verdict = HOLD on submit-now → founder picked "fix blockers + 15-min device test, then submit." FIXES APPLIED (commit adfcd79): (1) Settings Edit Profile now validates required fields before save (was the self-lockout: a cleared/invalid field wrote an incomplete profile that re-gated the user out — hard gate is ON for all since 2020). (2) Age floor 13→18 in the typed birth-year (onboarding + Settings). (3) Privacy Policy link on the onboarding consent step (Apple 5.1.1(ii)). (4) Selected-pill text→accentSoftText for dark-theme AA contrast. (5) persist() strips undefined so a mid-onboarding kill can't clobber earlier stashed answers. (6) goal/tracking pill clouds left-aligned. (7) legacy goal keys (e.g. athletic) render as removable pills, no silent invisible selection. (8) eas.json Android track internal→production (10% staged rollout, inProgress). (9) removed stale worktree angry-morse-9c0214 (had async onAuthStateChange — deadlock loaded gun). 174/175 tests, bundle exports clean. PRODUCTION BUILDS RUNNING: Android 413b2939, iOS ab964e4c (autoIncrement → build > 55, marketing 1.2.0). "What's New" drafted in all 6 langs (scratchpad/whats-new-1.2.0.txt). STILL OPEN / FOUNDER: (a) device-test the build before submit — QA min set: brand-new Apple sign-in clears the gate; returning account missing one field doesn't wipe the other 7; offline finish fails gracefully; edit-profile save round-trips; demo account appreview@dosetrace.io traverses the new onboarding. (b) F2 Apple token revocation on account delete (5.1.1(v)) — NOT implemented (Google is); needs storing the Apple refresh token + server revoke = 1.2.1 follow-up unless founder wants it now. (c) GDPR Art.9 explicit-consent wording for sex-at-birth + hormonal/sexual/cardio goals collected pre-signup — needs counsel (standing flag). SUBMIT MECHANICS: iOS = eas submit uploads only; then attach the new build to the 1.2.0 ASC version (currently DEVELOPER_REJECTED, editable), fill What's New ×6, Submit for Review via Chrome console (script writes blocked); verify WAITING_FOR_REVIEW via ASC API. Android = eas submit --profile production goes to production 10% staged; set per-locale release notes; confirm SA has Release-to-production permission.

**1.2.0 SUBMITTED TO BOTH STORES (2026-09-15, founder waived device test — "push it to production, even with small problems this is far better than 1.0.1").** iOS: eas submit uploaded build 56 (ab964e4c) → via Chrome ASC console removed build 55 from the 1.2.0 version, attached build 56, saved, Add for Review → Submit for Review. VERIFIED via ASC API: appStoreVersion 1.2.0 = WAITING_FOR_REVIEW. (What's New: EN set to the concise sign-in-first note; the other 5 localizations kept their existing valid rich notes — form_input reverts on ASC's React fields, and all 6 are non-empty so submission wasn't blocked; minor per-locale emphasis mismatch, not a correctness/policy issue.) Android: eas submit build 38 (413b2939) → production track, IN_PROGRESS, 10% staged rollout, changes sent for review. WEBSITE: web/ deployed to main (commit 4cd3487) → Vercel live; gallery desktop nav (arrows + click-drag) verified serving from dosetrace.io. STILL OPEN (founder): (a) Apple review ETA ~24-48h — watch for approval/rejection email; if rejected, fix + resubmit (no iOS rollback exists). (b) Android: watch Play review + vitals before ramping past 10%. (c) F2 Apple token revoke on delete → 1.2.1 follow-up. (d) GDPR Art.9 explicit-consent → counsel. NOTE: feat/wellness-rework is still NOT merged to main (only web/ was cherried to main); the app 1.2.0 ships from the EAS builds off the branch, but main still lacks the app code — a future full merge/PR is owed.

**1.2.0 APPROVED + LIVE ON APP STORE (2026-09-16).** Verified via ASC API: appStoreVersion 1.2.0 = READY_FOR_SALE, attached build 56. The live public iOS app now has the native Apple/Google sign-in fix + all the new layouts (About-you 4-col month grid + uppercase + section dividers, 18-goal multi-select, refreshed Settings/Edit Profile). NOTE: build 55 and build 56 BOTH report marketing version "1.2.0" (remote versioning) — the Settings "v1.2.0" string does NOT distinguish them. Founder's own device was showing the OLD layout because it runs the TestFlight build 55 (which overrides the App Store install); fix for his device = delete app + reinstall from the App Store to get build 56. The language-picker blank-rows fix (1b2748a) is NOT in build 56 (committed after the build) → still blank on live 1.2.0, queued for 1.2.1. Android: still production 10% staged (Play review separate from Apple).

**MAIN MERGED + 1.2.1 BACKLOG (2026-09-16).** PR #1 (feat/wellness-rework → main, 285 commits) merged (merge commit 3ad8858); feat/wellness-rework branch deleted (remote + local); main now = shipping 1.2.0 code. Stale local branch claude/angry-morse-9c0214 remains (harmless). **1.2.1 backlog (founder chose to batch, NOT reset 1.2.0's Apple review):** (1) DONE, on main (commit 1b2748a): onboarding language picker rendered blank rows — LANGUAGES objects are {code,name,native,flag} with no `label`, so {l.label} was undefined/invisible on the first screen; fixed to {l.native} (matches the working Settings picker). Founder reported it on his dark-theme device. (2) Apple token revocation on account delete (5.1.1(v)). (3) GDPR Art.9 explicit-consent wording (needs counsel). **1.2.1 BUILT + IN GATE (2026-09-18).** Founder: "build and ship everything, no pushing for later — include everything into 1.2.1." Ran dt-council (7 reviewers) = GO-WITH-CHANGES; founder calls: insulin loggable+curve-suppressed, halt Android 1.2.0 then ship 1.2.1 at 10%, full signup screen with resend. Applied ALL fixes (commits 8415278, 6df7ca7): blend MULTI-LINE curve (BLEND_RATIOS split — KLOW 80mg=50 GHK/10 KPV/10 TB/10 BPC per founder; each component charts its own line, dashed + estimated + "(est.)" label + reworded caveat pointing to log-components; pharma+regulatory re-check APPROVED); insulin CURVE_EXCLUDED (loggable only); half-life tiers fixed (Test Suspension→estimated); FeaturePreviews neutralized good/bad colors + light-theme SoftText contrast + neutral dates; signup success = Open-email + Resend(30s) + Wrong-address + auto-confirm guard; language-picker fix; Eloralintide + 9 compounds; app.json 1.2.0→1.2.1; theme rule hard-lined in CLAUDE.md. 174/175 tests, i18n parity green, bundle clean. BUILDS RUNNING: Android 22aafb36, iOS e189e704 (autoIncrement > 56, marketing 1.2.1). What's New drafted ×6 (scratchpad/whats-new-1.2.1.txt). SUBMIT STEPS OWED: (a) HALT the 1.2.0 Android rollout (still 10% inProgress) before submitting 1.2.1 — Play allows one active rollout/track; (b) iOS = eas submit uploads, then attach build to a NEW 1.2.1 ASC version + What's New ×6 + Submit for Review (Chrome console); (c) verify next build number actually > 56. FOLLOW-UP (founder wants, next build): blend ratio is currently fixed — add a per-vial ratio/dose EDITOR so users can adjust the split (the caveat now says "log components" instead of promising an adjust that doesn't exist).

(4) DONE, on main (commit 1afe7da): PAYWALL "show the moat" + conversion instrumentation — growth-marketer pass reshaped the founder's "preview every paid feature" idea. Growth found the real gaps: the paywall was all-text (no visual proof of the moat) and purchases.js fired NO analytics (couldn't measure conversion). Built the recommended slice: conversion events (paywall_viewed/cta_tapped/purchase_completed{plan,source}, preview_sheet_viewed) with a `source` on every navigate('Paywall'); paywall hero = the live AccumulationHero curve (Example-labeled) replacing the static icon; serum locked card (free) opens a Preview sheet (Example curve + Unlock-with-Pro) instead of jumping to paywall; bloodwork 2nd-upload upsell uses real scanned-marker count. Deliberately did NOT animate commodity features (sync/PDF/unlimited/tags/support). New i18n keys ×6. 174/175 tests, parity green, bundle clean. NOT visually verified on device (paywall unreachable on signed-out sim) — owed at ship-check Gate A both-themes + founder device test. NEEDS journey-review + dt-council + ship-check before the build (per CLAUDE.md — substantive paywall/flow change). 1.2.0 remains WAITING_FOR_REVIEW (iOS) / production 10% staged (Android).



**1.2.0 App Store version STAGED + CORRECTED (2026-09-14).** Founder mistyped the App Store version as 1.2.1 and submitted it with NO build (build 56 is marketing 1.2.0, couldn't attach to a 1.2.1 version → WAITING_FOR_REVIEW with no binary). Fixed via the ASC WEB CONSOLE (Claude-in-Chrome on the founder's logged-in session — the ASC API write path is blocked by the auto-mode classifier AND by 1.0.1 being READY_FOR_SALE): pulled 1.2.1 from review (Developer Rejected), renamed the Version field 1.2.1->1.2.0, saved. Build 55 (=build 56, VALID) then auto-linked; confirmed attached via GET /v1/appStoreVersions/{id}/build. State: DEVELOPER_REJECTED (editable, NOT submitted). en-US has 7 captioned screenshots + whatsNew in all 6 langs (other locales 0 screenshots -> Apple falls back to en-US). REMAINING: founder device-tests build 56 via TestFlight -> Add for Review -> Submit. HARD GATE holds: no Submit before the device test.

**STORE SCREENSHOT REFRESH + 1.2.0 PUBLIC RELEASE IN PROGRESS (2026-09-14).** Founder found the public App Store was still on **1.0.1** — the whole 1.2.x line (redesign→Journey→nutrition→Target, builds 51–55) had been TestFlight-only, never publicly released. Decision: release build 56 publicly as **1.2.0**. Build 56 = EAS build both platforms (iOS ASC build 55 = 1.2.0(55) VALID; Android vc37) — on internal tracks. Store screenshots refreshed to build 56 (7 captioned framed slides via store_assets headless-Chrome compositor: serum curve · calculator · target · today · protocols · my body · vaccines). **Google Play: DONE** — 7 shots uploaded to the (only) en-US listing via Android Publisher API (SA eas-play-submit has listing-edit; transactional commit), LIVE-pending. **Website: DONE (committed, deploy pending)** — serum curve added as flagship gallery figure + 5 shots refreshed + cap0 x6 langs. **Apple: screenshots uploaded by founder in ASC console** (API blocked: 1.0.1 READY_FOR_SALE 409; and the auto-mode classifier denies ASC writes — founder must do ASC writes in console or grant a Bash rule). Screenshot files staged at ~/Desktop/dosetrace-store-56/ (framed_apple_captioned.zip=1320×2868 for 6.5/6.7/6.9 slot; framed_apple_63_captioned.zip=1206×2622 for 6.3/6.1 slot). Release notes (6 langs) in scratchpad whatsnew.json. REMAINING for the 1.2.0 release (founder, in ASC console): attach build 1.2.0(55) + paste What's New + confirm demo acct appreview@dosetrace.io + EULA → **device-test build 56 via TestFlight** (wipe-guard + Target are why) → Add for Review → Submit. HARD GATE unchanged: no Submit for Review until founder device-tests build 56 clean.


**BUILD 56 — dt-council GO-WITH-CHANGES + ship-check PASS + BUILDING (2026-09-13, founder said build).** 10-role council ran (all GO-WITH-CHANGES). Fixes applied + committed before build: ETA needs >=14-day window (pharm block); rebased goal targets go through the floored goalsForTdee engine so the calorie floor holds on the measured path (pharm safety); below-healthy-range target guard = factual note + suppressed ETA (regulatory, App Store 1.4); Target gating reworked to FREE goal+progress / PREMIUM measured-ETA (founder call, matches the free-targets split); Save disabled until valid + backfill merges from a fresh DB read; updated_at trigger added to calc_targets + calc_snapshots (backend — cross-device edits now propagate; RLS verified live); calc_targets sync/mapper/floor tests; app-wide emoji->FeatureIcon sweep (22 new monoline glyphs + flame restored) with notification-title emoji stripped; cold-start cross-account notification-cancel fix (QA). 175 tests (174 pass, only known energy.ts), bundle clean, i18n parity green. ship-check A/B/C/D PASS (Gate B auth invariants verified in code; DEVICE TEST OWED before external). DEFERRED (founder-noted, low): multi-device duplicate target rows (accepted, no UNIQUE by design); stale sign-out flag extra timeout; no-rate inline CTA (copy only); backfillTakenDoses idempotency. OPEN (needs lawyer): below-range hard-floor / crisis-resource policy (stacks on the Art.9 flag). **NEXT DISCRETIONARY BUILD = the serum-curve MOAT coverage gap** (PM + growth call): ~47 pickable injectables still lack half-life data (only ~74 of 121+ covered per the build-48 handoff); the dose-accumulation/serum curve is the real differentiator vs Dose Track and must not keep losing ground to the weight-loss loop. Do NOT let Target become the pattern — enrich the curve next.

**BUILD 56 (calculator: reality-check wiring + personal Target) — CODE-COMPLETE on `feat/wellness-rework`, NOT built, NOT shipped (2026-09-13).** Founder-approved this session; queued for the next production build. NONE of it is on a device or in a build yet — before any EAS build: run dt-council → ship-check → founder device test (Gate B: this touches sync/storage) → then build. Commits: `aaf53b5` (reality-check drives daily burn + goal targets — measured maintenance becomes the hero number when a check exists, goals rebased off it, formula demoted to an "estimate" sub-line), `7ef185e` ("Why the daily burn moves" explainer — TDEE shifts with muscle/lean mass), `0bfe975` (**personal Target**). Target = new synced `calc_targets` table (SQLite+Supabase, RLS all-4 verified, tombstones, cloud table LIVE via apply_migration; parity file supabase_calc_targets_table.sql); one active target/user; weight and/or body-fat; values stored CANONICAL (kg,%) so a kg↔lb switch can't corrupt it (survives update/re-auth/sync per the never-lose-data rule). ETA = pure `targetProjection`/`seriesRatePerWeek` in lib/energyCalc.js, direction-aware from the set-time start (never a negative/wrong-way ETA), near-zero-rate guarded, BF% gets its OWN rate from BF snapshots, at/past = "reached", every ETA labelled with its weigh-in window, neutral healthy-range fact beside a weight target — all pure math, no advice. Backfill (founder call: "let them backfill past weigh-ins") = "+ Add a past weigh-in" writes a dated calc_snapshot (merges, never wipes other fields) so the rate/ETA appears without waiting 2 weeks. Journey-review ran BEFORE impl (findings F1–F9 all addressed; see the eval below). 170 tests (169 pass, only the known energy.ts fail) incl. 12 new target-projection tests; i18n parity green (6 langs, cal_tgt_* + cal_snap_bodyfat + cal_measured_from_check/cal_est + cal_expl_comp_*); iOS bundle exports clean. VERIFIED on simulator via jsbundle swap (measured-maintenance hero 2850, explainer, and a seeded Target rendering "87→82 kg · ~5 weeks · around Oct · based on weigh-ins Jul 14–Sep 8 · healthy range 58–78 kg"; seed then removed from cloud). OPEN FOUNDER DECISIONS (defaults chosen, flagged): (a) Target is PREMIUM-gated (aligned w/ reality-check/progress) — confirm; (b) stale-rate ETA is LABELLED with its window but not suppressed beyond that (no staleness cutoff) — confirm or set a cutoff; (c) BF% target needs BF% weigh-ins to show an ETA. Local sim demo account (f8ae7047) may still hold the seeded target row locally (synced, harmless).

**Last updated:** 2026-09-10 (late) — 🧪 **BUILD 51 (v1.2.0) ON INTERNAL TRACKS, awaiting founder device test.** iOS build 51 = VALID in ASC, in internal all-builds "Team (Expo)" group (installable now); verified via `/tf-status`. Android vc33 submitted to Play internal (⚠️ confirm the internal track is Active + tester list saved in Play Console — not API-verifiable here). **iOS build 50 EXPIRED** (founder call 2026-09-10, via ASC API PATCH; its external beta-review record cleared, freeing the Early Birds slot). Build 49 expired. **NOT YET on external/Early Birds** — submit 51 there only after the founder tests it clean on device. Build 49 expired earlier. All work on `feat/wellness-rework`. Tests 137/138 (1 pre-existing docs `.ts` fail). Verify releases with `/tf-status` — NEVER trust an `eas submit` exit code. [CLAUDE.md](CLAUDE.md) is the always-loaded operating contract; read it.

**Pending device test (blocks external submit):** auth flows carried from build 50's never-device-tested AuthScreen rework (sign-in email/Google/Apple, profile Save closes, delete→welcome, sign-out→welcome) + build-51 fixes (comma-decimal save round-trip in a EU locale, Android diluent water, first-dose-today loggable without dinging adherence, calc height field, curve combined-total estradiol label, vial scan). Founder tests via internal group; on "clean" → submit 51 to Early Birds external review + `/tf-status` verify.

**BUILD 52 (next) — scope + open founder decisions.** (1) **Calculator redesign** — DONE, committed a085d23 (grouped cards, section headers w/ new calc_trend/calc_bars/calc_bolt icons, compact activity grid, collapsed learn/sources, no emoji). (2) **AI conversational nutrition logger v1** — dt-council scoping done (6 roles, 2026-09-10), architecture converged, awaiting founder's calls before code. CONVERGED v1: extraction-ONLY structured JSON (never free chat/prose — the regulatory + injection + advice guard all in one); real RLS `food_logs` table via the existing syncCore engine (NOT user_metadata); own quota dimension (not the 3/mo scan cap), premium-gated + per-day cap + small max_tokens + fails-open + no raw-text server logging; offline-first (raw text saves immediately, AI parse is async enrichment); estimates/ranges only (≈), user-correctable; the rolling avg auto-fills the reality-check rcIntake (replaces the broken "recall 3 weeks of calories" input — this is the whole justification); code-enforced advice-refusal → fixed doctor/dietitian card ×6 langs; consent v3 + privacy-policy + delete-cascade + Apple/Play data-safety labels updated. CUT from v1: voice, nutrition DB, barcode, meal photos, multi-turn chat, macros beyond maybe protein, trends dashboard. FOUNDER DECISIONS (locked 2026-09-10): (a) BUILD into 52 — "no users, no cost if not used"; (b) ONE daily nudge at ~20:00 ("what did you eat today?"), evening-batch loop, per-submit parse capped a few/day + fails-open; (c) PREMIUM-only, but the locked state shows a self-running ANIMATED DEMO (AI asks → answer types in → a day's log fills) so the user sees what it is before paying; (d) lives in JOURNEY; the 8pm reminder fires ONLY while a reality-check window is ACTIVE (never bother a user who isn't running one); add an easy "Stop reality check" control → confirm "are you sure?" → deletes the reality-check numbers AND cancels the daily food reminders; (e) track Calories + Carbs + Protein (fat captured in JSON, not surfaced). LAWYER: food-text = same standing GDPR Art.9 flag (not new); confirm Anthropic DPA covers free-text. Positioning: NOT a food logger / MFP killer — intake-as-input next to the dose curve. Build order: [DONE ca181ad] data layer (food_logs table + SQLite mirror + syncCore/mappers, JSONB round-trip tests) → [DONE ac42f85] parse-food edge fn (deployed v1, tables applied to prod DB, smoke-tested; behavior = docs/nutrition-logger-conversation-spec.md, founder-validated live) → [DONE 42c31f6] Journey TAB (founder: "make the tab") — calculator promoted out of Body hub into its own tab (flag glyph, tab_journey x6, deep-links repointed, calc removed from Body = replace-not-bolt-on) → [DONE e9df448] nutrition logic (dayTotals/rollingAvgKcal/pickNudge + tests) → [DONE 91354ad] client data helpers (food_logs CRUD + parseFood invoke) → [DONE d75218e] nutrition logger UI + animated locked state, wired into Journey (Track section), 22 strings x6 (parity green) → [DONE 99d0756] wire food-log 7-day avg into reality-check (one-tap "use my food log" chip) → [DONE f77c73c] 8pm food reminder (only while a reality-check is active) + "Stop reality check" control (confirm → clears numbers + cancels reminders; keeps food log) → [DONE 1b5a546] consent v3 (meal text→Anthropic) + local delete-cascade (food_logs) + privacy-policy copy. **BUILD 52 IS CODE-COMPLETE.** Logger v1 notes: long-press-to-delete an entry (per-item portion edit is a fast-follow); premium CTA generic 'Unlock Premium' (no trial claim until one is configured in RevenueCat); daily food-reminder uses expo `trigger: {type:'daily',hour:20}` — unverified on device.

**BUILD 52 (v1.2.0 #52 / Android vc34) BUILT + ON INTERNAL TRACKS 2026-09-10.** iOS #52 = proc=VALID in ASC, internal "Team (Expo)" group (installable now), verified /tf-status. Android vc34 submitted to Play internal (⚠ confirm track Active + testers saved in Play Console). NOT external — waits for founder device test. Build 51 still shows external WAITING_FOR_REVIEW (superseded checkpoint; 52 ⊇ 51 — submitting 52 external will replace it). AWAITING FOUNDER DEVICE TEST (QA 12-step checklist: consent-v3 re-prompt across all AI scans; logger happy/refusal/offline; tap-to-fix; reality-check use-my-log chip + stop-control; Journey 5-tab render DE/PT). ship-check passed (Gate A/B/C/D). Then on "clean" → submit 52 Early Birds external + /tf-status.

**BUILD 52 — dt-council (full 10-role) DONE 2026-09-10: GO-WITH-CHANGES, no hard blockers.** All fixes applied + committed (8803322 + backend 2 fns redeployed): founder A=3-free-days-then-Premium, B=tap-to-fix edit modal, C=dose-accumulation curve card leads Journey. Council fixes: offline-reparse-on-every-focus (stale-closure bug), Reduce-Motion guard on the demo, 'use my food log' chip gated on >=5 logged days + 'estimated not measured', 8pm reminder scrolls to the logger, 'Food log'->'Daily intake' (anti-MFP), localized demo, parse-food count-before-call (redeployed v2), delete-user +food_logs/ai_food_usage (redeployed v10), privacy retention wording softened. 147/148 tests, parity green, bundle clean. Regulatory guardrails all verified in code. Device-test still owed (local iOS blocked).

**BUILD 52 — REMAINING BEFORE IT CAN SHIP (none are app code):**
1. **Pre-build gate**: run dt-council (full 10-role) → ship-check → EAS build both platforms → internal → FOUNDER DEVICE TEST (redesign, Journey tab, logger, 8pm reminder, reality-check wiring — ALL unverified on device, local iOS build blocked) → external. No build until founder says go.
2. **Deploy the updated privacy policy** to dosetrace.io (web/privacy-policy.html committed but not deployed).
3. **Dashboard (founder)**: Apple App-Privacy + Play Data-safety labels — add the nutrition/diet data type + Anthropic sharing. Confirm a 7-day trial product exists in RevenueCat if the CTA is to promise one.
4. **Lawyer**: Art.9 food-text classification + Anthropic DPA covers free-text (standing flag; gates STORE promotion, not TestFlight).
5. Build-51 device test still owed (separate, the current open blocker on shipping anything). NOTE premium demo concept approved by founder (artifact aa17535e), access model = openable anytime + one 8pm reminder, time-aware phrasing. NOTE: parse-food is deployed but not yet integration-tested with a real JWT (happens when UI wired / founder tests in-app). Premium gated CLIENT-side; server enforces 25/day cap as abuse ceiling.

**BUILD 53: large-screen / orientation support — DONE (founder call 2026-09-11 "do it correctly, take your time").** Portrait lock removed (`orientation: "default"`) + full responsive pass. Architecture: new `lib/responsive.js` exports `CONTENT_MAX_WIDTH=640` + `useResponsive()`; every scroll host now caps content to a 640pt centered column (`{width:'100%',maxWidth:640,alignSelf:'center'}` on the contentContainerStyle) so content never stretches edge-to-edge on a landscape phone / unfolded foldable / tablet — the cap never bites on a portrait phone. `SafeAreaProvider` added at the App.js root (with `initialWindowMetrics`) so insets track rotation (side-notch in landscape). Chart widths recomputed from `min(windowWidth, 640)` (BodyScreen, SerumCurveScreen, CalculatorSection) so they fit the capped column. ResetPasswordScreen wrapped in a ScrollView (was a fixed centered View — would clip in short landscape / with keyboard). Modals capped: SerumCurve picker sheet, NutritionLogger modalCard/demoCard, BodyMapModal sheet, all pageSheet modalBody (Body/Settings/Protocols/Vaccines). Onboarding splashWrap/centerStep minHeight removed (flexGrow centering handles portrait, no forced overflow in landscape). Grids (mChip/actChip/planRow) auto-fixed by the column cap. Files touched: app.json, App.js, lib/responsive.js + 13 screens + 5 components. Tests 149/150 (known energy.ts fail), bundle exported clean, all parse OK. **iOS `supportsTablet` kept FALSE** — native iPad is a separate store commitment (needs iPad screenshots + iPad review); flagged to founder as optional follow-up. Android foldables/tablets/landscape fully covered (Android has no supportsTablet gate). VERIFICATION OWED: on-device landscape/fold test (local iOS build blocked; expo-sqlite blocks web preview) — founder confirms the centered-column layout + rotation on the Z Fold. NOTE (out of scope, flagged): LogScreen empty-state uses a `📓` emoji — CLAUDE.md no-emoji violation, spawn a FeatureIcon fix. Sibling Play advisories (edge-to-edge window-color; manual image decode) are 3rd-party lib internals (Fresco/Glide/canhub-cropper), `edgeToEdgeEnabled:true` already set — clear via Expo SDK bumps, no app action.

**BUILD 55 (data-safety) — IN THE GATE (2026-09-12).** Founder directive: "make sure the entire app has a safe databank... I can't afford this type of error" + "spec it and take it through the gate for build 55." Spec: docs/data-safety-tables-spec.md. Moved the calculator's saved health history (reality checks + daily snapshots incl. waist) OFF Supabase user_metadata INTO synced Postgres+SQLite tables (reality_checks, calc_snapshots), owner-only RLS all 4 cmds (verified), tombstone deletes, one-time per-account migration gated on user_metadata.calc_history_migrated_v55 (non-destructive; metadata kept as backup). Commits 83a550b (impl) + 441899e (Gate B fixes). Gate B DONE (backend + senior deep reviews): fixed (1) migration-guard resurrection/dupe → durable flag; (2) userIdRef-null save no-op + toast-without-persistence → set every load + bail before toast; (3) read tables every focus; (4) dropped dead SNAP_CAP (history uncapped by design — capping = data loss). 153/154 tests (+mapper/round-trip/tombstone), bundle clean. STILL IN METADATA (deferred, cloud-backed so survive wipe, MED truncation risk): favorite_markers + report_tags (BodyScreen) — own follow-up build. calc_inputs (latest-wins object) acceptable. dt-council DONE: GO-WITH-CHANGES, all blocks fixed. qa BLOCK (offline-migration resurrection: cloud-only fire-and-forget flag) → durable LOCAL AsyncStorage flag MIGRATED_KEY (db3350f); release fail-open wrap; regulatory SQL parity files committed. release/regulatory APPROVE; product CONCERNS (sequencing + wipe-guard). DECISION PENDING (recommend defer to build 56): guard the App.js SIGNED_OUT wipe so a spurious sign-out can't wipe device data — now defense-in-depth only (all user data cloud-backed → wipe re-imports, no loss); Gate B danger (auth deadlock + anti-cross-account-leak) so do it carefully in its own build. ship-check PASS (A/B/C/D). **EAS build 55 all-platforms cut 2026-09-12 (bg bqraeyjc3)** — carries 53+54+55. HARD GATE before external: founder UPDATE-SURVIVAL test (enter reality-check + snapshot w/ waist on old build → update → both survive) + offline clear-then-relaunch-stays-cleared + sign-out/in no-dupes + landscape/fold matrix (qa checklist). Recommend enable Supabase PITR (dashboard). then verify /tf-status (carries 53+54+55) → founder device test incl. UPDATE-SURVIVAL (enter data on old build → update → data survives) + clear-then-restart-stays-cleared + landscape/fold → then external. Backups: recommend enabling Supabase PITR (founder dashboard action, not code).

**🔴 DATA-LOSS INCIDENT + FIX (2026-09-12) — build 53 HELD from external.** Founder lost an in-progress reality-check weigh-in on hello@dosetrace.io after updating. ROOT CAUSE: the weigh-in lived ONLY in AsyncStorage (RC_START_KEY), which the App.js SIGNED_OUT handler wipes and which never re-imports — a sign-out-class event on update cleared it. Diagnosed via SQL: account's synced data ALL INTACT (protocols 7, dose_logs 38, food_logs 7, biomarkers 47; calc_inputs/calc_snapshots present) — ONLY the device-only weigh-in was lost. NOT recoverable from server (never was in the cloud); it's a single number (start weight+date) to re-enter. FIX (68f6025): new lib/realityCheck.js cloud-backs the weigh-in into user_metadata.calc_reality_open (merge-safe) + restores the AsyncStorage cache after a wipe; CalculatorSection + TodayScreen route through it. New CLAUDE.md standing rule: NEVER lose user-entered data (durable synced/cloud-backed default; never destroy on a spurious sign-out; writes merge not clobber; migrate data on screen replace). AUDIT RUNNING (senior-eng) for any OTHER device-only/metadata-only user data with the same fragility. **Build 53 (on internal) does NOT go external; the fix + audit findings ride BUILD 54, cut after the audit + founder device test.** notifications.js still reads RC_START_KEY directly (reminder self-heals on screen load after a wipe — acceptable, not data). FOLLOW-UP (flagged, not done — Gate B risk): guard the App.js SIGNED_OUT wipe so it fires only on intentional sign-out (defense-in-depth; the wipe also serves anti-cross-account-leak, so change carefully).

**BUILD 53 — dt-council (full 10-role) DONE 2026-09-11: GO-WITH-CHANGES, one BLOCK fixed.** BLOCK (senior-eng, qa, ux): reparse() could SILENTLY delete a legit offline-saved meal on a one-off refusal/empty. FIXED (b9ca102): reparse deletes ONLY on explicit refusal; ambiguous 200/empty → parse_status='unparsed' (terminal, keeps raw_text, renders 'couldn't read — tap to remove'); onSubmit claims the row id in reparsingRef (kills the submit-vs-focus-reparse double-parse/double-quota race); typed text preserved on refusal/no-food; tap-to-fix now opens the (collapsed) history so the hint isn't a dead-end; logger scrims use c.overlay token. New string nutri_unparsed x6 (parity green). VERIFIED CLEAN: food_logs has owner DELETE RLS policy (checked via SQL — no resurrect bug); PK math untouched (charts cosmetic); AI hard-line intact (no new advice surface); onAuthStateChange unchanged/synchronous; alignSelf column pattern safe; iPad correctly left off. ORIENTATION DECISION: kept `default` (landscape on all iPhones too, matches founder's "every screen in landscape" ask) — shipped to INTERNAL only, device-test-owed; one-line revert to portrait available if founder prefers phones stay portrait. ship-check PASS (Gate A/B/C/D; both-theme VISUAL check device-test-owed — can't render RN locally). **EAS build all-platforms production kicked 2026-09-11 (bg bi8u5ueki).** After build: verify /tf-status (iOS grouped) + Android internal, then FOUNDER DEVICE TEST on the Z Fold + a normal phone (QA matrix: rotate/fold each tab, keyboard-up landscape on ResetPassword/Auth/composer, modals centered both themes, offline-real-food-survives test, longest-language layout) BEFORE any external push. DEFERRABLES (founder call, not blocking): ai_spark is the generic-AI sparkle used 4x + no unified chevron/back/close FeatureIcon system + loose type scale (own brand/icon-system pass); pre-existing UI emoji 📓 LogScreen (task chip) + 🔒 BodyScreen:702; instrument v24 gate funnel; NO 'tablet/foldable optimized' store copy until verified on a real large screen; drop stale app.json buildNumber:"2"; backend idea: cloud.delete .select('id') to self-heal silent 0-row deletes; 3 dead `|| '#d9544e'` fallbacks in CompleteProfileScreen (unreachable, reads both themes — trivial cleanup).

**AFTER BUILD 52 — founder request (2026-09-10): new store screenshots for BOTH Apple + Google.** The redesign + Journey tab + nutrition logger make the current store shots stale (ship-check GATE C: must show the REAL current app, no fabricated mockups). Source: capture from build 52 on the iOS simulator (retry the EAS simulator build — earlier upload to storage.googleapis.com kept dropping) OR founder device captures during testing; Apple needs 6.9" (1320x2868), Play copies padded to <=2:1. See [[ios-store-screenshots-and-build]]. Needed before the next STORE-review promotion, not for TestFlight.

**Build 51 = build 50's code + these fixes (commits 48e5d55, 5a35db9, 0cb7a26, 6c2d15f, 6538173, 993bc06, 98e7de8, 3d9e6d2):** Android diluent-water typeable; +34 sourced half-lives (curve 72→107/125); calc height field always visible; comma-locale SAVE bug (parseFloat→parseDecimal, was silently saving 1.0 for "1,5"); first-dose loggable-today WITHOUT dinging creation-day adherence (council-corrected); vial-scan error copy no longer says "lab-reading service"; extract-bloodwork **v12** deployed (provider/parse logging — root-caused + fixed the Anthropic-key-401 scan outage 2026-09-10); i18n substance_estradiol/progesterone added; Estradiol Cypionate citation + Progesterone tier corrected. Since 51 ⊇ 50, a clean 51 makes 50 moot.

**Build 50 fixes over 49:** sign-out/delete → the splash (was blank/old-onboarding), with no AuthScreen flash (QA-audit fix); legacy `OnboardingScreen` DELETED → lean `screens/AuthScreen.js`; vector icons on the compound step + new `type_glp1` glyph; repo cleanup (dead scratch, old edge-fn source, stale reports, superseded docs). Both a senior-eng (Gate B) and a routing-audit review ran and their findings were fixed. **Founder's iOS internal-tester record is `INVITED`/not-accepted** — he's a valid ADMIN user in the all-builds internal group (build 50 auto-included), so accepting the TestFlight invite on `jootaerre@yahoo.com.br` installs build 50 instantly (no review); the external link stays empty until Apple approves. Testing on the Z Fold 7 (Android) meanwhile. ⚠️ Verify the Play internal track is Active + tester list saved.
...
- Fresh install → first launch shows the **new intro** (not the welcome screen). In "About you", confirm you can't continue without **name + sex**. Finish → lands on auth screen.
- **Email sign-up** from the intro → after sign-in, open Settings/profile: name/goal/activity/**sex** written (verifies applyPendingProfile + no double-entry; the legacy signup step should be PREFILLED from the intro).
- **Google** + **Apple** fresh sign-in → both reach the app; Google account chooser appears.
- Edit profile → **Save closes** the screen. **Sign-out → returns to welcome.** **Delete account → returns to welcome.**
- **Sign out, then relaunch** → confirm you see the auth screen (via the intro's "Already have an account? Sign in" path), NOT a forced 8-step intro.
- **Calculator sex-gate:** with an account whose sex is unset, open Body → calculator, pick "body fat unknown" → result is withheld with a "set your sex" prompt; setting it unblocks + writes to profile. With body fat known, no gate.
- Existing **RTU protocol** card shows correct "X mg vial · N of M doses left".
- Switch to **2 non-English languages** → intro, `profile_sex_help`, `cal_sex_gate_*`, protocol card all translated (no raw keys).
- Corrected **half-lives** render sane curves: Test Undecanoate (now ~90d release, accumulates over months), Tesamorelin (minutes), L-carnitine (17.4h).
- Onboarding **curve shows an "Example" badge** and reads as a sawtooth (up-down per dose).

### 3. Serum-curve research (Phase 1 DONE) — decisions
- Covariate personalization is far thinner than hoped: **body-fat = 0 compounds**, **sex ≈ 0** on a serum-*level* curve (only oxytocin has real sex data, barely accumulates; HGH's sex effect is on IGF-1 response, not serum GH), **weight = 4 compounds** (the marketed GLP-1s). Full report artifact: https://claude.ai/code/artifact/094a97a1-73de-46ac-a17e-6abad7976f3c
- **Premium pivot:** the serum tool's value is accuracy + honest provenance + blood-draw-day cross-reference, NOT sex/fat toggles. Weight-on-GLP-1s is a small honest add.
- **PK model architecture** (adopted, founder + GPT): Vd/CL engine (`t½ = 0.693·Vd/CL`), per-compound `bodyCompositionPK` record, GREEN/YELLOW/RED evidence gate, covariate ≠ body-fat% (use the study's descriptor), implement published models only. Full spec in memory `pk-model-sex-and-modifiers.md`. This is a FUTURE build, not started.
- **Half-life flags resolved (commit `ed2bc89`, each label-verified):** FIXED 3 — Tesamorelin 0.6→0.13h, Testosterone Undecanoate 814→2160h, L-carnitine 10→17.4h. CLEARED as not-bugs — MK-677 (24h plasma t½ is real per Chapman 1996), Triptorelin (3h correct for the aqueous research bolus; Trelstar depot is a different product). Left as honest estimates — Sustanon (blend), IGF-1 LR3 (no human PK), insulin glargine (duration surrogate), Melanotan II (weakly sourced).
- **App has ~121 injectable drugs + ~40 oral supplements; only 74 have serum-curve half-life data** (~47 injectables are pickable but uncovered — a coverage gap for the redesign).

### 4. Onboarding build — commit map (`feat/wellness-rework`)
`43cff76` intro flow · `786c533` self-drawing curve · `fb97b7d` sawtooth · `6ebcb0a` sex-at-birth→BMR · `ed2bc89` half-life fixes · `8cb6ef7` cross-account leak fix · `31471ff` seenOnboarding race + 1.1.0 + honest AI/privacy copy + Example badge · `1b4d502` double-entry + sign-in escape · `b7cb338` calculator sex-gate · `0279d75` vector feature icons · `970f22b` reconstitution icon redraw · `7e49aca` eas simulator profile. Key files: `screens/OnboardingFlowScreen.js`, `lib/onboardingStore.js`, `App.js` (routing + SIGNED_OUT/SIGNED_IN), `components/AccumulationHero.js`, `components/FeatureIcon.js` + `components/featureIconsData.js` (+ `assets/feature-icons/*.svg`), `screens/CompleteProfileScreen.js`, `screens/components/CalculatorSection.js`, `lib/supabase.js` (`isProfileComplete`, `PROFILE_GATE_SINCE`), `lib/halfLives.js`, `i18n/translations.js`.
...
6. Minor: `birth_month` is stored 1-based by the intro but 0-based by the legacy signup screen — harmless today (nothing reads month), clean up if a feature starts using it.

### 6. How to continue (commands)
- Node PATH (bare Mac, nvm only): `export PATH="$HOME/.nvm/versions/node/v24.20.0/bin:$PATH"`.
- Tests: `npm test` (node --test; scope to `__tests__/` to skip the pre-existing docs `.ts` fail). Bundle check: `npx expo export --platform ios`.
- Rebuild/submit iOS: `eas build -p ios --profile production --non-interactive` then `eas submit -p ios --profile production --latest --non-interactive`. EAS logged in as jootaerre / hello@dosetrace.io.
- **External TestFlight (Early Birds) via ASC API** (eas submit only gets it to ASC): generate an ES256 JWT from the ASC key, then (a) `POST /v1/betaGroups/{groupId}/relationships/builds` to add the build, (b) `POST /v1/betaAppReviewSubmissions` to submit for external review. Params below.
- Local iOS **native build is BLOCKED** (CocoaPods 1.11.3 vs RN 0.81, Ruby 2.6). Use Expo Go for local (Metro `--offline`); EAS for real builds. Run `dt-council` before every build; then `ship-check`.

### 7. Infra pointers (NO secrets pasted — files stay on disk / out of git)
