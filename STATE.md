...
**Last updated:** 2026-09-09 (late²) — 🚀 **BUILD 50 (v1.2.0) SHIPPED to both stores; BUILD 49 EXPIRED.** iOS build 50 = VALID in ASC, attached to Early Birds + beta review submitted (WAITING_FOR_REVIEW, verified via `/tf-status`); awaiting Apple approval → then the public link serves 1.2.0. Android build 50 (versionCode auto-inc past 30) submitted to Play internal. Build 49 expired (superseded). All work on `feat/wellness-rework`. Tests 136/137 (1 pre-existing docs `.ts` fail). Verify releases with `/tf-status` — NEVER trust an `eas submit` exit code. [CLAUDE.md](CLAUDE.md) is the always-loaded operating contract; read it.
