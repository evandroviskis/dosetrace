...
**Last updated:** 2026-09-10 — 🧪 **BUILD 51 (v1.2.0) READY TO BUILD** (dt-council passed GO-WITH-CHANGES; all fixes landed + verified). **iOS build 50 is NOT live in production — it is `WAITING_FOR_REVIEW` in the Early Birds external queue (unapproved), verified via `/tf-status` 2026-09-10.** It occupies the single external-review slot; submitting 51 externally supersedes it. Founder can test any VALID build (50 or 51) NOW via the internal all-builds "Team (Expo)" group. Build 49 expired. All work on `feat/wellness-rework`. Tests 137/138 (1 pre-existing docs `.ts` fail). Verify releases with `/tf-status` — NEVER trust an `eas submit` exit code. [CLAUDE.md](CLAUDE.md) is the always-loaded operating contract; read it.
