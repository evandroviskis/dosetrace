...
const CLOUD_FIELDS = {
  protocols: [
    ...
    'serving_strength', 'serving_strength_unit', 'serving_units', 'container_units', 'units_taken',
    'active', 'deleted_at',
  ],
  ...
  vaccines: ['name', 'date_given', 'next_due', 'notes'],
};
...
function toCloudPayload(table, row) {
  ...
  if (table === 'protocols') {
    ...
    base.units_taken = row.units_taken;
