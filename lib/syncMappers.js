...
const CLOUD_FIELDS = {
  ...
  reality_checks: ['entry_date', 'tdee', 'rate_per_week_kg'],
  calc_snapshots: ['entry_date', 'weight_kg', 'waist_cm', 'body_fat_pct', 'lbm', 'bmr', 'tdee'],
  calc_targets: ['entry_date', 'target_weight_kg', 'target_body_fat_pct', 'target_date', 'start_date', 'start_weight_kg', 'start_body_fat_pct'],
};
...
function toCloudPayload(table, row) {
  ...
  if (table === 'protocols') {
    ...
    base.entry_date = row.entry_date;
    base.tdee = row.tdee;
    base.rate_per_week_kg = row.rate_per_week_kg;
  } else if (table === 'calc_snapshots') {
    base.entry_date = row.entry_date;
    base.weight_kg = row.weight_kg;
    base.waist_cm = row.waist_cm;
    base.body_fat_pct = row.body_fat_pct;
    base.lbm = row.lbm;
    base.bmr = row.bmr;
    base.tdee = row.tdee;
  } else if (table === 'calc_targets') {
    base.entry_date = row.entry_date;
    base.target_weight_kg = row.target_weight_kg;
    base.target_body_fat_pct = row.target_body_fat_pct;
    base.target_date = row.target_date;
    base.start_date = row.start_date;
    base.start_weight_kg = row.start_weight_kg;
    base.start_body_fat_pct = row.start_body_fat_pct;
  }
