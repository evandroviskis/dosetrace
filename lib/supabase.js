...
export async function signInWithGoogle() {
  ...
  if (!resultUrl) {
    // Distinguish a real browser-launch failure from a user cancellation so the
    // diagnostic message points at the actual cause.
    return { error: browserErr
      ? { message: `Browser failed to open: ${browserErr.message || browserErr}` }
