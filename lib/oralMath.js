...
function computeServings(inp) {
  ...
  // Whether THIS product can be split — a user override of the form's default.
  const splittable = (inp.divisible === true || inp.divisible === false)
    ? inp.divisible
    : formInfo.splittable;
  ...
  // Achievable step: continuous forms (powder/liquid) allow any amount; a
  // splittable (scored/notched) discrete unit can be cut to HALVES; otherwise
  // only whole units. `nearest` brackets by that step when the target doesn't
  // land on it.
  const step = !formInfo.discrete ? 0 : (splittable ? 0.5 : 1);
  const isAchievable = step === 0
    ? true
    : Math.abs(Math.round(unitsNeeded / step) - unitsNeeded / step) < 1e-9;

  let nearest = null;
  if (step > 0 && !isAchievable) {
    const low = Math.floor(unitsNeeded / step + 1e-9) * step;
    const high = Math.ceil(unitsNeeded / step - 1e-9) * step;
    nearest = {
  ...
  return {
    ...
    isAchievable,
    step,
    perUnitDose: round(perUnit, 3),   // strength of one physical unit, in doseUnit
    ratio: round(perUnit / targetDose, 2), // how many of the user's doses one unit holds
    nearest,
  };
}
