...
export async function redeemReferralCode(code, newUserId) {
  try {
    ...
    if (!rpcError && rpcData) {
      if (rpcData.success) {
        await grantReferralCredit(code);
        return { success: true };
      }
...
// Grant a bloodwork credit to the freshly-referred user.
// TODO(server): bloodwork_credits lives in client-writable user_metadata,
// so it can be forged by any client. Move credit granting/accounting
// server-side (Edge Function + RLS-protected table).
async function grantReferralCredit(code) {
  try {
