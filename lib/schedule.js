...
function expectedDosesOn(p, dayDate) {
  ...
  // NOTE (2026-09-10): a protocol that STARTS today must keep today's dose(s)
  // loggable, even if the reminder time already passed when it was created — a
  // user sets it up after taking the first dose and needs to mark it taken.
  // "Loggable today" is decoupled from "fire a reminder": the notification layer
  // (scheduleDoseReminder) skips already-passed slots, so no late alarm fires,
  // but the dose still shows on Today as due-and-loggable. (Previously a same-day
  // slot created >1h after its time was zeroed out, hiding the first dose.)
  return dpd;
}
