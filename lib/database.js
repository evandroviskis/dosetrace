...
export function initDatabase() {
  ...
  // Migration: add concentration_unit column for existing databases
  try { db.execSync(`ALTER TABLE protocols ADD COLUMN concentration_unit TEXT DEFAULT 'mg';`); } catch (e) { /* column already exists */ }

  // Migration: add diluent column (what the compound was reconstituted with)
  ...
  // Migration: add injection_site column for existing databases
...
export function insertProtocol(data) {
  ...
  const result = db.runSync(
    `INSERT INTO protocols (remote_id, user_id, name, type, color, amount, unit, water, diluent, dose, dose_unit, syringe_size, concentration, concentration_unit, frequency, reminder_time, interval_days, doses_per_day, start_date, schedule_total, goal, notes, active, created_at, updated_at, sync_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, 'pending')`,
    [
...
export function getPendingChanges(table, userId) {
  const db = getDB();
  return db.getAllSync(
...
export function markSynced(table, id, remoteId, expectedUpdatedAt, cloudUpdatedAt) {
  const db = getDB();
  if (remoteId) {
    // Always record the remote_id so a mid-flight edit still maps to the cloud row
...
export function deleteLocalRow(table, id) {
  const db = getDB();
    ...
    if (table === 'protocols') {
      db.runSync(
        ...
        [
          ...
          row.amount, row.unit, row.water, row.diluent || null, row.dose, row.dose_unit,
          row.syringe_size, row.concentration, row.concentration_unit || 'mg', row.frequency, row.reminder_time,
          row.interval_days || 1, row.doses_per_day || 1,
          row.start_date, row.schedule_total, row.goal, row.notes,
          row.active ? 1 : 0, row.deleted_at, row.created_at, row.updated_at || now,
        ]
      );
    } else if (table === 'vials') {
      ...
      db.runSync(
        ...
        [
          ...
          row.active ? 1 : 0, row.created_at, row.updated_at || now,
        ]
      );
      ...
      db.runSync(
        `INSERT INTO dose_logs (remote_id, user_id, protocol_id, protocol_remote_id, outcome, injection_site, logged_at, updated_at, sync_status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'synced')`,
        [row.id, row.user_id, localP?.id || null, row.protocol_id, row.outcome, row.injection_site || null, row.logged_at, row.updated_at || now]
      );
    } else if (table === 'biomarkers') {
      db.runSync(
        `INSERT INTO biomarkers (remote_id, user_id, report_date, marker, value, unit, created_at, updated_at, sync_status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'synced')`,
        [row.id, row.user_id, row.report_date, row.marker, row.value, row.unit, row.created_at, row.updated_at || now]
      );
    }
  }
}
