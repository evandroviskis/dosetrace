...
export function insertProtocol(data) {
  ...
  const result = db.runSync(
    `INSERT INTO protocols (remote_id, user_id, name, type, color, amount, unit, water, dose, dose_unit, syringe_size, concentration, concentration_unit, frequency, reminder_time, interval_days, doses_per_day, start_date, schedule_total, goal, notes, active, created_at, updated_at, sync_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, 'pending')`,
    [
...
export function markSynced(table, id, remoteId, expectedUpdatedAt) {
  const db = getDB();
  if (remoteId) {
...
export function importFromCloud(table, rows) {
  ...
  for (const row of rows) {
    ...
    if (table === 'protocols') {
      db.runSync(
        `INSERT INTO protocols (remote_id, user_id, name, type, color, amount, unit, water, dose, dose_unit, syringe_size, concentration, concentration_unit, frequency, reminder_time, interval_days, doses_per_day, start_date, schedule_total, goal, notes, active, deleted_at, created_at, updated_at, sync_status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'synced')`,
        [
      ...
      db.runSync(
        `INSERT INTO dose_logs (remote_id, user_id, protocol_id, protocol_remote_id, outcome, injection_site, logged_at, updated_at, sync_status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'synced')`,
        [row.id, row.user_id, localP?.id || null, row.protocol_id, row.outcome, row.injection_site || null, row.logged_at, now]
      );
