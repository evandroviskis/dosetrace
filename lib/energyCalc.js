...
// Three floored goals for ANY maintenance value — used both by energyPlan (from
// the formula TDEE) and by the UI when a reality-check MEASURED maintenance
// replaces the formula, so the safety floor applies on the measured path too
// (else scaling lose calories by measured/formula can silently dip below 1200).
function goalsForTdee(tdeeVal, { bmr = 0, sex } = {}) {
  const absFloor = sex === 'male' || sex === 'female' ? CALORIE_FLOOR[sex] : 0;
  ...
  return {
    lose: clampLose(goalCalories(tdeeVal, 'lose')),
...
function energyPlan(inp) {
  ...
  const allGoals = goalsForTdee(tdeeVal, { bmr, sex: inp.sex });
...
// Pure display math for a user's target (weight or body-fat). Everything is
// direction-aware and guarded so the ETA is never misleading:
//   • never a NEGATIVE or wrong-direction ETA (F1: the rate sign convention is
//     "+ means the value went DOWN", which flips naively for a gain goal),
//   • never a divide-by-near-zero blow-up (F2: a flat/maintenance trend),
//   • a metric with no usable rate yet returns 'no_rate' (F3/F4) — progress is
//     still shown by the caller; the ETA is simply absent, never fabricated,
//   • at/past the target is a first-class 'reached' state (F7),
//   • a trend moving AWAY from the target is 'away' — honest, no number.
// current/target/ratePerWeek share one unit (kg or %). ratePerWeek follows the
// reality-check convention: POSITIVE = the value is falling per week.

// "close enough to count as reached" band, per metric kind.
const TARGET_BAND = { weight: 0.2, bodyfat: 0.3 };
// below this |rate| the trend is too flat to project (aligns with the reality-
// check 0.2 kg maintenance band over a ~2-week window ≈ 0.1 kg/week).
const TARGET_MIN_RATE = { weight: 0.1, bodyfat: 0.1 };
// the weigh-in series must span at least this many days before an ETA is shown —
// otherwise ordinary day-to-day water/glycogen swings inflate the rate into a
// confident-but-wrong timeline (e.g. −1 kg over 2 days ⇒ "~2 weeks to goal").
// Matches the reality-check's own 14-day minimum window.
const TARGET_MIN_WINDOW_DAYS = 14;

// `start` is the value when the target was set; it fixes the goal DIRECTION
// (a target below the start is a loss goal, above it a gain goal) so an overshoot
// reads as 'reached' rather than flipping to 'away'. When start is absent the
// direction falls back to the rate sign, then to current-vs-target.
function targetProjection({ current, target, start = null, ratePerWeek, kind = 'weight' }) {
  if (current == null || target == null) return { state: 'no_rate', gap: null, etaWeeks: null };
  const band = TARGET_BAND[kind] ?? 0.2;
  const gap = current - target;             // >0 above target; <0 below target
  let goingDown;                            // is the goal to LOWER the value?
  if (start != null) goingDown = target < start;
  else if (ratePerWeek != null && ratePerWeek !== 0) goingDown = ratePerWeek > 0;
  else goingDown = target < current;
  // Reached: within the band, or already crossed the target in the goal direction.
  const crossed = goingDown ? current <= target + band : current >= target - band;
  if (Math.abs(gap) <= band || crossed) return { state: 'reached', gap, etaWeeks: null };
  const floor = TARGET_MIN_RATE[kind] ?? 0.1;
  if (ratePerWeek == null || Math.abs(ratePerWeek) < floor) return { state: 'no_rate', gap, etaWeeks: null };
  // Speed at which the gap is CLOSING in the goal direction (rate: + = falling).
  const progressRate = goingDown ? ratePerWeek : -ratePerWeek;
  if (progressRate <= 0) return { state: 'away', gap, etaWeeks: null };
  return { state: 'eta', gap, etaWeeks: Math.abs(gap) / progressRate };
}

// Whole-day difference between two 'YYYY-MM-DD' dates (b − a). Parsed as UTC noon
...
// Weekly rate from a dated series [{ date:'YYYY-MM-DD', value:Number }], measured
// first vs last (same basis as the reality check: start vs now). POSITIVE = the
// value went DOWN. Returns null with <2 usable points or a zero-length window, so
// the caller shows progress-only instead of a fake rate (F3/F4). Also reports the
// window so the ETA can be labelled with the dates it is based on (F8).
function seriesRatePerWeek(points) {
  const pts = (points || [])
    .filter(p => p && p.date && typeof p.value === 'number' && !Number.isNaN(p.value))
  ...
  if (!(days > 0)) return null;
  return {
    ratePerWeek: ((first.value - last.value) / days) * 7,
    days,
...
module.exports = {
  ...
  weeklyRateKg, targetProjection, seriesRatePerWeek, daysBetweenISO,
  goalsForTdee, TARGET_BAND, TARGET_MIN_RATE, TARGET_MIN_WINDOW_DAYS,
