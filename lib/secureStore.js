...
const CHUNK_SIZE = 1800;          // < SecureStore's ~2048-byte limit (multibyte headroom)
...
async function scWrite(k, value) {
  ...
  for (let i = 0; i < chunks.length; i++) await SecureStore.setItemAsync(slotChunkKey(k, target, i), chunks[i]);
  // Atomic pointer flip — a reader now sees the fully-written target slot.
  await SecureStore.setItemAsync(metaKey(k), `${target}:${chunks.length}:${value.length}`);
