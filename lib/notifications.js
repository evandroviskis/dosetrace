...
export async function syncRealityCheckReminder() {
  ...
  try {
    ...
    let start = null;
    try { const raw = await AsyncStorage.getItem(RC_START_KEY); start = raw ? JSON.parse(raw) : null; } catch { start = null; }
...
// Daily 20:00 "what did you eat today?" nudge for the AI food log. Founder rule:
// it fires ONLY while a reality-check is active (an open RC_START_KEY), so a user
// who isn't running one is never bothered. Repeats daily; cleared when the
// reality-check stops. The food log itself is openable anytime — this is just the
// catch-all reminder for people who didn't log during the day.
export async function syncFoodLogReminder() {
  const N = getNotifications();
  if (!N) return;
  try {
    const all = await N.getAllScheduledNotificationsAsync();
    for (const n of all.filter(x => x.identifier === 'food-log-daily')) {
      await N.cancelScheduledNotificationAsync(n.identifier);
    }
    let start = null;
    try { const raw = await AsyncStorage.getItem(RC_START_KEY); start = raw ? JSON.parse(raw) : null; } catch { start = null; }
...
export async function registerPushToken() {
  ...
  try {
    ...
    await supabase.from('push_tokens').upsert(
      { user_id: user.id, expo_token: token, platform: Platform.OS, updated_at: new Date().toISOString() },
