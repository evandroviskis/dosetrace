...
// Extract the fire date from a one-shot scheduled trigger, if determinable.
// Android date triggers come back as { type: 'date', value }, iOS as a
// calendar trigger with dateComponents. Repeating triggers return null.
function getTriggerFireDate(trigger) {
  if (!trigger || trigger.repeats) return null;
  if (trigger.type === 'date') {
    const v = trigger.value ?? trigger.date;
    return v != null ? new Date(v) : null;
  }
  if (trigger.type === 'calendar' && trigger.dateComponents) {
    const c = trigger.dateComponents;
    if (c.year != null && c.month != null && c.day != null) {
      return new Date(c.year, c.month - 1, c.day, c.hour ?? 0, c.minute ?? 0);
    }
  }
  return null;
}
...
export async function cancelFollowups(protocolId) {
  ...
  try {
    ...
    const followups = all.filter(n =>
      n.identifier.startsWith(`dose-${protocolId}-`) && n.identifier.includes('-f')
    );
    const today = new Date();
    for (const n of followups) {
      // Only cancel one-shot follow-ups firing today — future days' follow-ups
      // and repeating daily triggers must stay scheduled
      const fireDate = getTriggerFireDate(n.trigger);
      if (!fireDate) continue;
      if (
        fireDate.getFullYear() === today.getFullYear() &&
        fireDate.getMonth() === today.getMonth() &&
        fireDate.getDate() === today.getDate()
      ) {
...
export async function syncMorningSummary() {
  ...
  try {
    ...
    // Cancel existing before rescheduling (e.g. after a language change)
    const all = await N.getAllScheduledNotificationsAsync();
    for (const n of all.filter(x => x.identifier === 'morning-summary')) {
      await N.cancelScheduledNotificationAsync(n.identifier);
    }
