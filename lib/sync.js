        ...
        if (freshRow.remote_id) {
          ...
          if (!error) {
            if (updated && updated.length > 0) {
              markSynced(table, freshRow.id, freshRow.remote_id, freshRow.updated_at);
            } else {
              // Cloud row no longer exists — re-create it so the data isn't lost
              const { data, error: insertError } = await supabase
                ...
                .single();
              if (!insertError && data) {
                markSynced(table, freshRow.id, data.id, freshRow.updated_at);
                if (table === 'protocols') {
                  updateChildRemoteIds(freshRow.id, data.id, true);
                }
              }
            }
          }
...
function updateLocalFromCloud(table, localId, cloudRow) {
  ...
  if (table === 'protocols') {
    db.runSync(
      `UPDATE protocols SET name = ?, type = ?, color = ?, amount = ?, unit = ?, water = ?, dose = ?, dose_unit = ?, syringe_size = ?, concentration = ?, concentration_unit = ?, frequency = ?, reminder_time = ?, interval_days = ?, doses_per_day = ?, start_date = ?, schedule_total = ?, goal = ?, notes = ?, active = ?, deleted_at = ?, updated_at = ?, sync_status = 'synced' WHERE id = ?`,
      [
    ...
    db.runSync(
      ...
      [localP?.id || null, cloudRow.protocol_id, cloudRow.outcome, cloudRow.injection_site || null, cloudRow.logged_at, now, localId]
    );
  } else if (table === 'biomarkers') {
    db.runSync(
  ...
  if (table === 'protocols') {
    ...
    db.runSync(
      `INSERT INTO dose_logs (remote_id, user_id, protocol_id, protocol_remote_id, outcome, injection_site, logged_at, updated_at, sync_status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'synced')`,
      [cloudRow.id, cloudRow.user_id, localP?.id || null, cloudRow.protocol_id, cloudRow.outcome, cloudRow.injection_site || null, cloudRow.logged_at, now]
    );
  } else if (table === 'biomarkers') {
    db.runSync(
